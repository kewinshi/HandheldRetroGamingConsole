...
void displayStartup() {
    ...
    // Display Offset Coordinate Test (commented out)
...
//     drawSafeZones(); // Testing safe zones (commented out)

    // Always display the header and menu options
    initStarField();
    ...
    while (1) {
        ...
        if (input == 2) { // Up button
            if (game_choice != 0) {
                ...
                game_choice = (game_choice - 1) % 4;
            } else {
                prev_gamechoice = game_choice;
                game_choice = 3; // Wrap to Space Invaders
            }
        ...
        if (game_choice == 0 && input != -1) {
            ...
            clear_area(140, 8 * LINE_HEIGHT, 170, 14 * LINE_HEIGHT, BLACK); // Clear below
            ...
            clear_area(140, 10 * LINE_HEIGHT, 170, 14 * LINE_HEIGHT, BLACK); // Clear below
            ...
            clear_area(140, 12 * LINE_HEIGHT, 170, 14 * LINE_HEIGHT, BLACK); // Clear below
        } else if (game_choice == 3 && input != -1) {
            LCD_DrawImage(110, 12.7 * LINE_HEIGHT, 1, 3); // Space Invaders logo (assumed index 3)
